# -*- coding: utf-8 -*-
"""Churn Analysis Using Machine Learning.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1_QImOroZa-Qsy5Km9_ASoiBjPonuf2Mg
"""
...
"""After uploading the `Churn_Modelling.csv` file, you can then load it using `pd.read_csv('Churn_Modelling.csv')`."""
...
sns.heatmap(df.drop(['RowNumber','CustomerId'],axis=1).corr(numeric_only=True), annot=True, fmt='.2f')
plt.show()
...
correlation_matrix = df.drop(['RowNumber','CustomerId'],axis=1).corr(numeric_only=True)
target_correlations = correlation_matrix['Exited']
print(target_correlations)

"""#Observation:

- Age shows a moderately positive correlation, suggesting older customers are more likely to churn

- IsActiveMember has a negative correlation, indicating that active members are less likely to churn

- High CreditUtilization and NumOfProducts also appear to be associated with a higher likelihood of churning
...
for name, model in models.items():
    ...
    new_row = pd.DataFrame([{'Model': name, 'Accuracy': accuracy, 'Recall Score': recall, 'F1 Score': f1, 'ROC AUC Score': roc_auc}])
